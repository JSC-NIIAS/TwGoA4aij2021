...
def train_model(hyp,opt,device,wandb):
    ...
    dataloader, dataset = create_dataloader(X_train,y_train, batch_size,
                                            hyp=experiment_dict['dataset'], mode="train",
    ...
    if rank in [-1, 0]:
        ...
        testloader,testset = create_dataloader(X_test,y_test, batch_size,
                                            hyp=experiment_dict['dataset'], mode="test",
                                            rank=rank, world_size=opt.world_size, workers=opt.workers)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss = loss_function.forward(pred,targets.to(device))
                if rank != -1:
        ...
        if results_dict['mean_F1_val'] >= best_fitness:
            best_fitness = results_dict['mean_F1_val']
